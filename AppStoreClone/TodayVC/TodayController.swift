        ...
        dispatchGroup.notify(queue: .main) {
            ...
            self.todayItems = [
                TodayItem(category: "Daily List",
                          title: topPaidGames?.feed.title ?? "",
                          image: UIImage(named: "garden")!,
                          description: "",
                          backgroungColor: .white,
                          cellStyle: .multiply,
                          appsResult: topPaidGames?.feed.results ?? []),
                TodayItem(category: "LIFE HACK",
                          title: "Utilazing your Time",
                          image: UIImage(named: "garden")!,
                          description: "All the tools and apps you need to intelligently organize your life in the right way.",
                          backgroungColor: .white,
                          cellStyle: .single, appsResult: []),
                TodayItem(category: "Daily List",
                          title: topFreeGames?.feed.title ?? "",
                          image: UIImage(named: "garden")!,
                          description: "",
                          backgroungColor: .white,
                          cellStyle: .multiply,
                          appsResult: topFreeGames?.feed.results ?? []),
                TodayItem(category: "HOLIDAY",
                          title: "Travel on a Badget",
                          image: UIImage(named: "holiday")!,
                          description: "Find out all you need to know on how to travel without packing everything!",
                          backgroungColor: .init(named: "yellow")!,
                          cellStyle: .single,
                          appsResult: [])
            ]
...
// MARK: - CollectionView methods
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId,
                                                      for: indexPath) as! BaseTodayCell
...
// MARK: - Dismiss Methods
    ...
    @objc fileprivate func dismissFullView() {
        UIView.animate(withDuration: 0.7, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.7, options: .curveEaseOut, animations: {
